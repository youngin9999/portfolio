...
def ip(b , capacity_matrix):
    ...
    model.Minimize(1.01 *x[0] + 1.01 *x[1] + 1.01* x[2] + 1.01*x[3] + 1.02 *x[4] + 1.02 *x[5] + 1.02 *x[6] +x[7] )
...
def load_original_distance(path="C:\Contest\Cj\distance-data.txt"):
    ...
    for dest in reversed(route):
        ...
        for  o_type , o in orders:
            if o["destination"] == dest[:-2]  and o_type == int(dest[-1]): 
                ...
                # y축을 트럭 뒤쪽(y=0)부터 앞으로(y=truck_y) 쌓이도록 반전
                truck_y = 280  # 트럭 y길이(깊이), 기존 값과 동일하게 맞춰주세요
                cy_raw = (count_idx[box_type] // max_y[box_type]) * dy[box_type]
                cy = truck_y - dy[box_type] - cy_raw
                cx = present_x_coord_by_box_type[box_type]
                cz = (count_idx[box_type] % max_y[box_type]) * dz[box_type]
...
def solve_vrp(mat, node_to_idx, idx_to_name, dem0, dem1, dem2,
              vehicle_types, capacity_matrix, time_limit=20):
